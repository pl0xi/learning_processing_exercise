public class Cat {
  ...
  private PShape cat;
  Cat(float x_, float y_, float size_) {
    x = x_;
    y = y_;
    size = size_;
    sizeLegs = size/4;
  ...
  private PShape createCat() {
    PShape cat_ = createShape(GROUP);
    cat_.addChild(quads(size/2, 0, -size, widthBoxLegs, sizeLegs));
    cat_.addChild(quads(-size/2, 0, -size, widthBoxLegs, sizeLegs));
    cat_.addChild(quads(size/2, widthBox-widthBoxLegs, -size, widthBoxLegs, sizeLegs));
    cat_.addChild(quads(-size/2, widthBox-widthBoxLegs, -size, widthBoxLegs, sizeLegs));
    cat_.addChild(quads(0, 0, 0, widthBox, size));
  ...
  private PShape tail() {
    PShape tail_ = createShape(BOX, 5, 5, widthBox);
    return tail_;
  }
